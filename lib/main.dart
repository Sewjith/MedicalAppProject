    ...
    return MultiBlocProvider(
      ...
      child: BlocListener<AuthBloc, AuthState>(
        listener: (context, state) {
          ...
          debugPrint("--- AuthBloc Listener Received State: ${state.runtimeType} ---");

          if (state is AuthSuccess) {
            final receivedRole = state.role; // Store role locally for clarity
            final userEmail = state.user.email;
            debugPrint("AuthSuccess Details: Role='${receivedRole}', Email='${userEmail}'");
            ...
            if (receivedRole == 'patient') {
              debugPrint(">>> Role MATCHED 'patient'. Navigating to /p_dashboard via appRouter.go");
              ...
              debugPrint(">>> Role MATCHED 'doctor'. Navigating to /d_dashboard via appRouter.go");
              Future.microtask(() => appRouter.go('/d_dashboard'));
            } else {
              debugPrint(">>> Role did NOT match 'patient' or 'doctor' (was '${receivedRole}'). Navigating to /home via appRouter.go");
