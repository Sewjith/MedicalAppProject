...
class NotificationService {
  ...
  void listenToNotifications({
    ...
    _notificationChannel!
        .onPostgresChanges(
            ...
            callback: (payload) {
              print(
                  'Realtime Notification Payload Received: ${payload.toString()}');
              if (payload.newRecord != null) {
                final newItem = payload.newRecord!;
                if (newItem['receiver_type'] == receiverType &&
                    newItem['receiver_id'] == receiverId) {
                  onNewNotification(
                    NotificationItem(
                  ...
                  print(
                      "Filtered out notification for different receiver/type: ID ${newItem['receiver_id']}, Type ${newItem['receiver_type']}");
                }
              } else {
                print(
                    "Received Realtime payload without newRecord: ${payload.eventType}");
              }
            })
        .subscribe(
      (status, [error]) {
        if (status == RealtimeSubscribeStatus.subscribed) {
          print(
              'Successfully subscribed to notification channel: $channelName');
        } else {
          print(
              'Notification subscription failed for $channelName: Status $status, Error: $error');
        }
      },
    );
  }
  ...
  Future<void> markAsRead(String id) async {
    try {
      await _client
          .from('notifications')
          .update({'read_status': true}).eq('id', id);
    } catch (e) {
  ...
  Future<void> markAsUnread(String id) async {
    try {
      await _client
          .from('notifications')
          .update({'read_status': false}).eq('id', id);
    } catch (e) {
  ...
  Future<void> deleteAllNotifications(
      String receiverType, String receiverId) async {
    try {
      await _client
          .from('notifications')
          .delete()
